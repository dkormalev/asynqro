...
Asynqro requires C++17 and Qt5 (should work with almost any version older than 5.6, but something newer is always recommended). It also uses CMake as build system and downloads GoogleTest to build dir if tests are enabled in build.
...
So why not to create another one? Future-related part of asynqro contains next classes:
- `Promise`
- `Future`
- `CancelableFuture`
...
- `onSuccess` - `(T->void)->Future<T>` adds a callback for successful case. If Future is already filled it will be called immediately (the same is for all other transformations).

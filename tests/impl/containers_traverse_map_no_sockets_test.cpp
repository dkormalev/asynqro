...
TEST(ZeroSocketedInputContainersMapTest, mapQByteArrayEmpty)
{
    ...
    QVector<int> result = traverse::map(emptyContainer, [](char x) -> int { return (x - '0') * 2; }, QVector<int>());
...
TEST(ZeroSocketedInputContainersMapTest, mapQByteArray)
{
    ...
    QVector<int> result = traverse::map(testContainer, [](char x) { return (x - '0') * 2; }, QVector<int>());
...
{
    QByteArray testContainer = "123456789";
    QMap<int, bool> result = traverse::map(testContainer, [](char x) { return qMakePair((x - '0') * 2, (x - '0') % 2); },
                                           QMap<int, bool>());
    ASSERT_EQ(9, result.size());
    for (int i = 1; i <= 9; ++i) {
        int key = i * 2;
...
{
    QByteArray testContainer = "123456789";
    QVector<long long> result = traverse::map(testContainer, [](long long index, char x) { return (x - '0') * index; },
                                              QVector<long long>());
    ASSERT_EQ(9, result.size());
    for (int i = 0; i < 9; ++i)
...
TEST(ZeroSocketedInputContainersMapTest, mapQJsonArrayEmpty)
{
    ...
    QVector<int> result = traverse::map(emptyContainer, [](const QJsonValue &x) -> int { return x.toInt() * 2; },
                                        QVector<int>());
    EXPECT_EQ(0, result.size());
}
...
TEST(ZeroSocketedInputContainersMapTest, mapQJsonArray)
{
    ...
    QVector<int> result = traverse::map(testContainer, [](const QJsonValue &x) -> int { return x.toInt() * 2; },
                                        QVector<int>());
    ASSERT_EQ(9, result.size());
    for (int i = 1; i <= 9; ++i)
        EXPECT_EQ(i * 2, result[i - 1]) << i;
...
{
    QJsonArray testContainer = QJsonArray::fromVariantList({1, 2, 3, 4, 5, 6, 7, 8, 9});
    QMap<int, bool> result = traverse::map(testContainer,
                                           [](const QJsonValue &x) { return qMakePair(x.toInt() * 2, x.toInt() % 2); },
                                           QMap<int, bool>());
    ASSERT_EQ(9, result.size());
    for (int i = 1; i <= 9; ++i) {
        int key = i * 2;
...
{
    QJsonArray testContainer = QJsonArray::fromVariantList({1, 2, 3, 4, 5, 6, 7, 8, 9});
    QVector<long long> result = traverse::map(testContainer,
                                              [](long long index, const QJsonValue &x) { return x.toInt() * index; },
                                              QVector<long long>());
    ASSERT_EQ(9, result.size());
    for (int i = 0; i < 9; ++i)
        EXPECT_EQ(i * (i + 1), result[i]) << i;
